document.addEventListener("DOMContentLoaded", () => {
  ...
  mask($("#campoTelefone"), v => {
    v = v.replace(/\D/g, "").slice(0, 11);
    return v.replace(/^(\d{2})(\d)/, "($1) $2").replace(/(\d)(\d{4})$/, "$1-$2");
  });
  mask($("#campoCep"), v => {
  ...
  // ATUALIZA INTERFACE DO CABEÇALHO DO PERFIL
  function carregarHeader() {
    const inicial = (usuarioLogado.nomeCompleto || usuarioLogado.usuario || "U").charAt(0).toUpperCase();
  ...
  // PREENCHE OS CAMPOS DE DADOS CADASTRAIS
  ...
  $$(".aba-item").forEach(btn => {
    btn.onclick = () => {
      $$(".aba-item").forEach(b => b.classList.remove("ativa"));
      $$(".painel-aba").forEach(p => p.classList.remove("ativo"));
  ...
  $("#formDadosPessoais").onsubmit = e => {
    ...
    usuarioLogado.usuario = $("#campoUsuario").value.trim();
    usuarioLogado.nomeCompleto = $("#campoNomeCompleto").value.trim();
    usuarioLogado.cpf = $("#campoCpf").value.trim();
    usuarioLogado.nascimento = $("#campoNascimento").value;
    usuarioLogado.telefone = $("#campoTelefone").value.trim();
    usuarioLogado.cep = $("#campoCep").value.trim();

    // Atualiza a lista geral no localStorage
    const index = usuarios.findIndex(u => u.usuario === usuarioAntigo);
    if (index >= 0) {
  ...
  $("#formEndereco").onsubmit = e => {
    ...
    msg.textContent = "Endereço principal salvo!";
    setTimeout(() => msg.textContent = "", 3500);
  };
  ...
  $("#formSenha").onsubmit = e => {
    ...
    if (atual !== usuarioLogado.senha) {
      msg.textContent = "A senha atual está incorreta.";
    ...
    if (index >= 0) usuarios[index].senha = nova;
  ...
  // CARREGAR HISTÓRICO DE PEDIDOS DEMONSTRATIVOS
  function carregarPedidos() {
    ...
    box.innerHTML = `
      <div class="card-pedido">
        ...
        <div class="total-pedido">Total: R$ 63,90</div>
      </div>
      ...
      <div class="card-pedido">
        ...
        <div class="total-pedido">Total: R$ 84,70</div>
      </div>
    `;
  }

  // CARREGAR FAVORITOS NA ABA DE FAVORITOS
  function carregarFavoritosPerfil() {
    const box = $("#containerFavoritosPerfil");
    const favs = JSON.parse(localStorage.getItem("favoritos") || "[]");

    if (typeof produtos === "undefined" || !favs.length) {
      box.innerHTML = `<p style="color:#777; grid-column:1/-1;">Você ainda não possui produtos favoritados.</p>`;
      return;
    }
    ...
    box.innerHTML = lista.map(p => `
      <div class="mini-card-fav">
        ...
        <a href="../produto/index.html?id=${p.id}" class="btn-ver-fav">Ver Produto</a>
      </div>
    `).join("");
  }
